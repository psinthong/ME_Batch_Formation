# python mem_benchmark.py --num_features=1000 --seq_len=1000 --time_steps=100000 --mode='regular' --n_epoch=10 --device='cuda' --batch_size=500
...
def train_regular_one_epoch(args, dl, device):
    start = time.time()
    runtimes=[]
...
if __name__ == '__main__':
    if "SLURM_PROCID" in os.environ: rank = int(os.environ["SLURM_PROCID"])
    if "WORLD_SIZE" in os.environ: world_size = int(os.environ["WORLD_SIZE"])
    ...
    for i in range(args.n_epoch):
        ...
        if args.mode == 'optimize':
            ...
            dl = DataLoader(dset, batch_size=None, collate_fn=sw.slide_collate_fn, pin_memory=False)
            ...
            sset = DummyPretrainStackDataset(data_x, data_y, num_features, time_steps, seq_len=args.seq_len, pred_len=pred_len,
                                             label_len=label_len)
            dl = DataLoader(sset, batch_size=args.batch_size, shuffle=False, drop_last=True)

        runtime = train_regular_one_epoch(args, dl, device)
        end = time.time()
        duration = end - start

        if num_epoch > warmup_epoch:
            total_runtime += duration

        if args.mode == 'regular':
            print('Stacked ||  duration: ', duration, ' runtime: ',runtime)
        elif args.mode == 'optimize_in_mem':
            print('Optimize in memory ||  duration: ', duration, ' runtime: ',runtime)
        else:
            print('Optimize || duration: ', duration, ' runtime: ',runtime)

    print("Number of batches: ", len(dl))
    print('Total runtime for the 1st batch: ', total_runtime/(args.n_epoch - warmup_epoch), ' batch-only: ', runtime)
